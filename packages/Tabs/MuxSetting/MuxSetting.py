...
from packages.Tabs.MuxSetting.Widgets.MakeThisAudioDefaultCheckBox import (
    MakeThisAudioDefaultCheckBox,
)
from packages.Tabs.MuxSetting.Widgets.MakeThisSubtitleDefaultCheckBox import (
    MakeThisSubtitleDefaultCheckBox,
)
from packages.Tabs.MuxSetting.Widgets.MakeThisTrackDefaultComboBox import (
...
from packages.Tabs.MuxSetting.Widgets.OnlyKeepThoseSubtitlesCheckBox import (
    OnlyKeepThoseSubtitlesCheckBox,
)
...
def check_if_at_least_one_muxing_setting_has_been_selected(window_parent):
    if (
        check_is_there_subtitle_to_mux()
        or check_is_there_audio_to_mux()
        or len(GlobalSetting.ATTACHMENT_FILES_LIST) > 0
        or len(GlobalSetting.ATTACHMENT_PATH_DATA_LIST) > 0
        or len(GlobalSetting.CHAPTER_FILES_LIST) > 0
        or GlobalSetting.CHAPTER_DISCARD_OLD
        or GlobalSetting.ATTACHMENT_DISCARD_OLD
        ...
        or GlobalSetting.MUX_SETTING_MAKE_THIS_AUDIO_DEFAULT_SEMI_ENABLED
        or GlobalSetting.MUX_SETTING_MAKE_THIS_AUDIO_DEFAULT_FULL_ENABLED
        or GlobalSetting.MUX_SETTING_MAKE_THIS_SUBTITLE_DEFAULT_SEMI_ENABLED
        or GlobalSetting.MUX_SETTING_MAKE_THIS_SUBTITLE_DEFAULT_FULL_ENABLED
        or GlobalSetting.VIDEO_OLD_TRACKS_VIDEOS_MODIFIED_ACTIVATED
        or GlobalSetting.VIDEO_OLD_TRACKS_AUDIOS_MODIFIED_ACTIVATED
        or GlobalSetting.VIDEO_OLD_TRACKS_SUBTITLES_MODIFIED_ACTIVATED
...
class MuxSettingTab(QWidget):
    ...
    def connect_signals(self):
        ...
        self.make_this_audio_default_checkBox.disable_combo_box.connect(
            self.disable_make_this_audio_default_comboBox
        )
        ...
        self.control_queue_button.add_to_queue_clicked_signal.connect(
            self.add_to_queue_button_clicked
        )
        self.control_queue_button.start_multiplexing_clicked_signal.connect(
            self.start_multiplexing_button_clicked
        )
        self.control_queue_button.pause_multiplexing_clicked_signal.connect(
            self.pause_multiplexing_button_clicked
        )

        self.clear_job_queue_button.clicked.connect(self.clear_job_queue_button_clicked)

        self.only_keep_those_audios_multi_choose_comboBox.closeList.connect(
            self.only_keep_those_audios_close_list
        )
        ...
        self.output_as_mks_checkBox.stateChanged.connect(
            self.output_as_mks_state_changed
        )
    ...
    def setup_add_crc_checksum_checkBox(self):
        ...
        self.add_crc_checksum_checkBox.setToolTip(
            "Add CRC checksum to the end of output file's name"
        )
    ...
    def setup_destination_path_lineEdit(self):
        self.destination_path_lineEdit.setPlaceholderText(
            "Enter Destination Folder Path [Leave Empty For Overwrite " "Source Files]"
        )
    ...
    def check_destination_path(self):
        ...
        try:
            ...
            if os.name == "nt":
                if (
                    temp_destination_path[1:3] != ":\\"
                    and self.destination_path_lineEdit.text()[1:3] != ":/"
            ...
            test_file_name_absolute = os.path.join(
                Path(temp_destination_path), Path(test_file_name)
            )
            try:
                ...
                write_to_log_file(e)
                invalid_dialog = InvalidPathDialog(
                    window_title="Permission Denied",
                ...
                for job in self.job_queue_layout.table.data:
                    if not job.done or (
                        ...
                        free_space = shutil.disk_usage(
                            path=os.path.dirname(job.video_name_absolute)
                        ).free
    ...
    def setup_tracks_to_be_chosen_mkv_only_options(self):
        ...
        self.make_this_subtitle_default_comboBox.refresh_tracks(
            new_list=GlobalSetting.MUX_SETTING_SUBTITLE_TRACKS_LIST
        )
        self.make_this_audio_default_comboBox.refresh_tracks(
        ...
        if GlobalSetting.JOB_QUEUE_EMPTY:
            if (
                ...
                if GlobalSetting.VIDEO_OLD_TRACKS_SUBTITLES_MODIFIED_ACTIVATED:
                    disable_reason = (
                        "Because you have modified some subtitle tracks in <b>Modify Old Tracks</b> "
                        "option in Video Tab "
                    )
                ...
                if GlobalSetting.VIDEO_OLD_TRACKS_AUDIOS_MODIFIED_ACTIVATED:
                    disable_reason = (
                        "Because you have modified some audio tracks in <b>Modify Old Tracks</b> option "
                        "in Video Tab "
                    )
    ...
    def enable_editable_widgets(self):
        ...
        self.only_keep_those_audios_checkBox.setEnabled(True)
        self.only_keep_those_audios_multi_choose_comboBox.setEnabled(
            self.only_keep_those_audios_checkBox.isChecked()
        )
        self.make_this_subtitle_default_checkBox.setEnabled(True)
        self.make_this_subtitle_default_comboBox.setEnabled(
            self.make_this_subtitle_default_checkBox.isChecked()
        )
        self.make_this_audio_default_checkBox.setEnabled(True)
        self.make_this_audio_default_comboBox.setEnabled(
            self.make_this_audio_default_checkBox.isChecked()
        )
    ...
    def only_keep_those_audios_close_list(self):
        GlobalSetting.MUX_SETTING_ONLY_KEEP_THOSE_AUDIOS_TRACKS_LANGUAGES = (
            self.only_keep_those_audios_multi_choose_comboBox.tracks_language
        )
        GlobalSetting.MUX_SETTING_ONLY_KEEP_THOSE_AUDIOS_TRACKS_IDS = (
            self.only_keep_those_audios_multi_choose_comboBox.tracks_id
        )
        GlobalSetting.MUX_SETTING_ONLY_KEEP_THOSE_AUDIOS_TRACKS_NAMES = (
            self.only_keep_those_audios_multi_choose_comboBox.tracks_name
        )

    def only_keep_those_subtitles_close_list(self):
        GlobalSetting.MUX_SETTING_ONLY_KEEP_THOSE_SUBTITLES_TRACKS_LANGUAGES = (
            self.only_keep_those_subtitles_multi_choose_comboBox.tracks_language
        )
        GlobalSetting.MUX_SETTING_ONLY_KEEP_THOSE_SUBTITLES_TRACKS_IDS = (
            self.only_keep_those_subtitles_multi_choose_comboBox.tracks_id
        )
        GlobalSetting.MUX_SETTING_ONLY_KEEP_THOSE_SUBTITLES_TRACKS_NAMES = (
            self.only_keep_those_subtitles_multi_choose_comboBox.tracks_name
        )
    ...
    def check_if_all_input_videos_are_found(self):
        for video_file in GlobalSetting.VIDEO_FILES_ABSOLUTE_PATH_LIST:
            if not Path.is_file(Path(video_file)):
                invalid_dialog = FileNotFoundDialog(
