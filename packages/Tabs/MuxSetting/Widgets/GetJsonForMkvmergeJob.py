...
class GetJsonForMkvmergeJob:
    ...
    def generate_info_file(self):
        ...
        for track in self.tracks_json_info:
            ...
            new_track_info.is_forced = str(
                get_attribute(
                    data=track["properties"], attribute="forced_track", default_value="no"
                )
            )
            new_track_info.language = str(
                get_attribute(
                    data=track["properties"], attribute="language", default_value="eng"
                )
            )
        ...
        for attachment in self.json_info["attachments"]:
            ...
            new_attachment_info.file_name = str(
                get_attribute(
                    data=attachment, attribute="file_name", default_value="no_name.ttf"
                )
            )
            new_attachment_info.id = str(
                get_attribute(data=attachment, attribute="id", default_value="-1")
            )
            new_attachment_info.size = get_attribute(
                data=attachment, attribute="size", default_value=0
            )
        ...
        if len(self.job.attachments_absolute_path) > 0:
            ...
            for file_to_attach in self.job.attachments_absolute_path:
                ...
                attachments_list_with_attach_command.append(
                    add_json_line("--attach-file")
                )
                attachments_list_with_attach_command.append(
                    add_json_line(check_for_system_backslash_path(file_to_attach))
                )
            self.attachments_attach_command = "".join(
                attachments_list_with_attach_command
            )
    ...
    def setup_video_default_duration_fps_command(self):
        if GlobalSetting.VIDEO_DEFAULT_DURATION_FPS not in ["", "Default"]:
            self.video_default_duration_fps_command = add_json_line(
                "--default-duration"
            ) + add_json_line("0:" + GlobalSetting.VIDEO_DEFAULT_DURATION_FPS)

    def setup_video_track_order(self):
        if GlobalSetting.VIDEO_OLD_TRACKS_VIDEOS_REORDER_ACTIVATED:
            for bulk_track in sorted(
                GlobalSetting.VIDEO_OLD_TRACKS_VIDEOS_BULK_SETTING.values(),
            ...
            for track_id in GlobalSetting.VIDEO_OLD_TRACKS_VIDEOS_BULK_SETTING.keys():
                ...
                if bulk_track.language != "[Old]":
                    ...
                    old_video_command_list.append(
                        add_json_line(
                            f"{track_id}:" + ISO_639_2_LANGUAGES[bulk_track.language]
                        )
                    )
                ...
                if bulk_track.track_name != "[Old]":
                    ...
                    old_video_command_list.append(
                        add_json_line(f"{track_id}:" + bulk_track.track_name)
                    )
    ...
    def setup_old_subtitle_tracks_options(self):
        ...
        if GlobalSetting.VIDEO_OLD_TRACKS_SUBTITLES_MODIFIED_ACTIVATED:
            for track_id in GlobalSetting.VIDEO_OLD_TRACKS_SUBTITLES_BULK_SETTING.keys():
                bulk_track = GlobalSetting.VIDEO_OLD_TRACKS_SUBTITLES_BULK_SETTING[
                    track_id
                ]
                if not bulk_track.is_enabled:
                    continue
                ...
                if bulk_track.is_default == 2:
                    old_subtitle_command_list.append(
                        add_json_line("--default-track-flag")
                    )
                ...
                if bulk_track.is_forced == 2:
                    old_subtitle_command_list.append(
                        add_json_line("--forced-display-flag")
                    )
                    old_subtitle_command_list.append(add_json_line(f"{track_id}:yes"))
                elif bulk_track.is_forced == 0:
                    old_subtitle_command_list.append(
            ...
            if self.job.subtitle_found:
                for i in range(len(self.job.subtitle_name_absolute)):
                    ...
                    subtitle_command_list.append(
                        add_json_line(
                            "0:" + ISO_639_2_LANGUAGES[self.job.subtitle_language[i]]
                        )
                    )
                    # add subtitle track name
                    if self.job.subtitle_track_name != "":
                        ...
                        subtitle_command_list.append(
                            add_json_line("0:" + self.job.subtitle_track_name[i])
                        )
                    # add subtitle set default
                    ...
                    subtitle_command_list.append(
                        add_json_line("0:" + str(subtitle_delay_in_millisecond))
                    )
                    # add subtitle file
                    subtitle_command_list.append(add_json_line("("))
                    subtitle_command_list.append(
                        add_json_line(
                            check_for_system_backslash_path(
                                self.job.subtitle_name_absolute[i]
                            )
                        )
                    )
                    subtitle_command_list.append(add_json_line(")"))
    ...
    def setup_subtitle_track_order(self):
        if GlobalSetting.VIDEO_OLD_TRACKS_SUBTITLES_REORDER_ACTIVATED:
            ...
            for i in range(len(self.job.subtitle_name_absolute)):
                ...
                self.current_track_index += 1
            for bulk_track in sorted(
                GlobalSetting.VIDEO_OLD_TRACKS_SUBTITLES_BULK_SETTING.values(),
                key=operator.attrgetter("order"),
            ):
                if not bulk_track.is_enabled:
                    continue
            ...
            for i in range(len(self.job.subtitle_name_absolute)):
                if self.job.subtitle_set_at_top[i] != -1:
                    tracks_order_list.append(
                        (
                            self.job.subtitle_set_at_top[i],
                            False,
                            int(i),
                            str(self.current_track_index) + ":0,",
                        )
                    )
                else:
                    tracks_order_list.append(
                    ...
                    if subtitle_track.id == bulk_track.id:
                        tracks_order_list.append(
                            (
                                order_id,
                                True,
                                int(subtitle_track.id),
                                "0:" + str(subtitle_track.id) + ",",
                            )
                        )
                        order_id += 1
                        break
            ...
            if self.job.audio_found:
                for i in range(len(self.job.audio_name_absolute)):
                    ...
                    audio_command_list.append(
                        add_json_line(
                            "0:" + ISO_639_2_LANGUAGES[self.job.audio_language[i]]
                        )
                    )
                    # add audio track name
                    if self.job.audio_track_name != "":
                        ...
                        audio_command_list.append(
                            add_json_line("0:" + self.job.audio_track_name[i])
                        )
                    # add audio set default
                    ...
                    audio_command_list.append(
                        add_json_line("0:" + str(audio_delay_in_millisecond))
                    )
                    # add audio file
                    ...
                    audio_command_list.append(
                        add_json_line(
                            check_for_system_backslash_path(
                                self.job.audio_name_absolute[i]
                            )
                        )
                    )
                    audio_command_list.append(add_json_line(")"))
    ...
    def setup_audio_track_order(self):
        if GlobalSetting.VIDEO_OLD_TRACKS_AUDIOS_REORDER_ACTIVATED:
            ...
            for i in range(len(self.job.audio_name_absolute)):
                ...
                self.current_track_index += 1
            for bulk_track in sorted(
                GlobalSetting.VIDEO_OLD_TRACKS_AUDIOS_BULK_SETTING.values(),
                key=operator.attrgetter("order"),
            ):
                if not bulk_track.is_enabled:
                    continue
    ...
    def setup_which_old_subtitles_to_keep(self):
        if GlobalSetting.MUX_SETTING_ONLY_KEEP_THOSE_SUBTITLES_ENABLED:
            if (
                len(GlobalSetting.MUX_SETTING_ONLY_KEEP_THOSE_SUBTITLES_TRACKS_LANGUAGES)
                == 0
                and len(GlobalSetting.MUX_SETTING_ONLY_KEEP_THOSE_SUBTITLES_TRACKS_IDS)
                == 0
                and len(GlobalSetting.MUX_SETTING_ONLY_KEEP_THOSE_SUBTITLES_TRACKS_NAMES)
                == 0
            ):
                ...
                only_keep_those_subtitle_list = (
                    GlobalSetting.MUX_SETTING_ONLY_KEEP_THOSE_SUBTITLES_TRACKS_IDS.copy()
                )
                for (
                    track_name
                ...
                only_keep_those_subtitle_list = list(
                    dict.fromkeys(only_keep_those_subtitle_list)
                )
    ...
    def setup_which_old_audios_to_keep(self):
        if GlobalSetting.MUX_SETTING_ONLY_KEEP_THOSE_AUDIOS_ENABLED:
            if (
                len(GlobalSetting.MUX_SETTING_ONLY_KEEP_THOSE_AUDIOS_TRACKS_LANGUAGES)
                == 0
                and len(GlobalSetting.MUX_SETTING_ONLY_KEEP_THOSE_AUDIOS_TRACKS_IDS) == 0
                and len(GlobalSetting.MUX_SETTING_ONLY_KEEP_THOSE_AUDIOS_TRACKS_NAMES)
                == 0
            ):
                ...
                only_keep_those_audios_list = (
                    GlobalSetting.MUX_SETTING_ONLY_KEEP_THOSE_AUDIOS_TRACKS_IDS.copy()
                )
                for (
                    track_name
                ) in GlobalSetting.MUX_SETTING_ONLY_KEEP_THOSE_AUDIOS_TRACKS_NAMES:
                ...
                for (
                    audio
                ) in GlobalSetting.MUX_SETTING_ONLY_KEEP_THOSE_AUDIOS_TRACKS_LANGUAGES:
                    only_keep_those_audios_list.append(ISO_639_2_LANGUAGES[audio])
                only_keep_those_audios_list = list(
                    dict.fromkeys(only_keep_those_audios_list)
                )
                ...
                if subtitle_track == "":
                    for subtitle in self.subtitles_track_json_info:
                        change_default_subtitle_commands_list.append(
                            add_json_line("--default-track")
                        )
                        change_default_subtitle_commands_list.append(
                            add_json_line(subtitle.id + ":no")
                        )
                    self.change_default_forced_subtitle_track_setting_source_video_command += "".join(
                        change_default_subtitle_commands_list
                    )
                ...
                if subtitle_track == "":
                    ...
                    if track_type == "id":
                        ...
                        if found_subtitle_with_this_language:
                            for subtitle in self.subtitles_track_json_info:
                                if (
                                    subtitle.language == subtitle_track_language
                                    and subtitle_track_id == ""
                                ):
                                    subtitle_track_id = subtitle.id
                                    change_default_subtitle_commands_list.append(
                                        add_json_line("--default-track")
                        ...
                        if found_subtitle_with_this_track_name:
                            for subtitle in self.subtitles_track_json_info:
                                if (
                                    subtitle.track_name == track_value
                                    and subtitle_track_id == ""
                                ):
                                    subtitle_track_id = subtitle.id
                                    change_default_subtitle_commands_list.append(
            ...
            if not audio_track.isspace():
                ...
                if audio_track == "":
                    ...
                    if audio_track_id != "":
                        ...
                        change_default_audio_commands_list.append(
                            add_json_line(audio_track_id + ":yes")
                        )
                        for audio in self.audios_track_json_info:
                            if audio.id != audio_track_id:
                                change_default_audio_commands_list.append(
                ...
                if audio_track == "":
                    ...
                    if track_type == "id":
                        ...
                        if found_audio_with_this_language:
                            for audio in self.audios_track_json_info:
                                if (
                                    audio.language == audio_track_language
                                    and audio_track_id == ""
                                ):
                                    audio_track_id = audio.id
                                    change_default_audio_commands_list.append(
    ...
    def setup_output_video_command(self):
        ...
        output_video_commands_list.append(
            add_json_line(check_for_system_backslash_path(output_video_name_absolute))
        )
