...
def generate_tool_tip_for_audio_file(
    audio_full_path="C:/Test",
    audio_name="Test",
    audio_delay=0.0,
    audio_language=Options.CurrentPreset.Default_Audio_Language,
...
def generate_tool_tip_for_subtitle_file(
    subtitle_full_path="C:/Test",
    subtitle_name="Test",
    subtitle_delay=0.0,
...
def set_attachments_setting_for_job(new_job, new_row_id):
    ...
    if GlobalSetting.ATTACHMENT_EXPERT_MODE:
        if len(GlobalSetting.ATTACHMENT_PATH_DATA_LIST) > new_row_id:
            new_job.attachments_absolute_path = GlobalSetting.ATTACHMENT_PATH_DATA_LIST[
                new_row_id
            ].files_list.copy()
    else:
...
def get_file_name_without_crc(file_name):
    ...
    if (bracket_right_index - bracket_left_index - 1) == 8:
        ...
        if valid_counter == 8:
            file_name_without_crc = (
                file_name[:bracket_left_index] + file_name[bracket_right_index + 1 :]
            )
    ...
    for i in range(len(file_name_without_extension) - 1, -1, -1):
        if file_name_without_extension[i] != " ":
            ...
            break
    file_name_without_crc = (
        file_name_without_crc[: last_valid_char + 1]
        + file_name_without_crc[extension_index:]
    )
...
class JobQueueTable(TableWidget):
    ...
    def set_row_value_size_after_muxing(self, finished_job: SingleJobData, row_index):
        ...
        if output_video_size_bytes == 0:
            self.delete_video_output_with_zero_size(
                file_path=finished_job.output_video_absolute_path
            )
        size_after_muxing = " " + get_readable_filesize(output_video_size_bytes)
        self.setCellWidget(
            row_index, self.column_ids["Size After"], QLabel(size_after_muxing)
        )
    ...
    def set_row_value_subtitle(self, new_job, new_row_id):
        ...
        for i in GlobalSetting.SUBTITLE_FILES_LIST.keys():
            if len(GlobalSetting.SUBTITLE_FILES_LIST[i]) > new_row_id:
                ...
                new_job.subtitle_name_absolute.append(
                    GlobalSetting.SUBTITLE_FILES_ABSOLUTE_PATH_LIST[i][new_row_id]
                )
    ...
    def set_row_value_audio(self, new_job, new_row_id):
        ...
        for i in GlobalSetting.AUDIO_FILES_LIST.keys():
            if len(GlobalSetting.AUDIO_FILES_LIST[i]) > new_row_id:
                ...
                new_job.audio_name_absolute.append(
                    GlobalSetting.AUDIO_FILES_ABSOLUTE_PATH_LIST[i][new_row_id]
                )
    ...
    def set_row_value_progressBar(self, new_job, new_row_id):
        new_job.progress = 0
        self.setCellWidget(
            new_row_id, self.column_ids["Progress"], ProgressBar(value=new_job.progress)
        )
    ...
    def resize_name_column_to_fit_content(self):
        ...
        for i in range(self.rowCount()):
            self.data[i].video_name_displayed = (
                chr(0x200E) + self.data[i].video_name_with_spaces
            )
    ...
    def cell_double_clicked(self, row_index, column_index):
        if column_index == self.column_ids["Subtitle"]:
            if self.data[row_index].subtitle_found:
                ...
                for i in GlobalSetting.SUBTITLE_FILES_LIST.keys():
                    if len(GlobalSetting.SUBTITLE_FILES_LIST[i]) > row_index:
                        subtitles_default_value_delay_list.append(
                            GlobalSetting.SUBTITLE_DELAY[i]
                        )
                        subtitles_default_value_language.append(
                            GlobalSetting.SUBTITLE_LANGUAGE[i]
                        )
                        subtitles_default_value_track_name.append(
                            GlobalSetting.SUBTITLE_TRACK_NAME[i]
                        )
                        subtitles_default_value_set_default.append(
                            GlobalSetting.SUBTITLE_SET_DEFAULT[i]
                        )
                        subtitles_default_value_set_forced.append(
                            GlobalSetting.SUBTITLE_SET_FORCED[i]
                        )
                ...
                if subtitle_info_dialog.state == "yes":
                    ...
                    current_cell_widget = self.cellWidget(
                        row_index, self.column_ids["Subtitle"]
                    )
                    if len(subtitle_info_dialog.current_subtitle_delay) == 1:
                        current_cell_widget.update_tool_tip(
                            tool_tip=generate_tool_tip_for_subtitle_file(
                                ...
                                subtitle_set_default=self.data[
                                    row_index
                                ].subtitle_set_default[0],
                                subtitle_set_forced=self.data[
                                    row_index
                                ].subtitle_set_forced[0],
                                show_full_path=False,
                            )
                        )
                ...
                warning_dialog = WarningDialog(
                    window_title="Subtitle Info",
                    info_message="No subtitle found!",
                    parent=self,
                )
            ...
            if self.data[row_index].audio_found:
                ...
                for i in GlobalSetting.AUDIO_FILES_LIST.keys():
                    if len(GlobalSetting.AUDIO_FILES_LIST[i]) > row_index:
                        audios_default_value_delay_list.append(
                            GlobalSetting.AUDIO_DELAY[i]
                        )
                        audios_default_value_language.append(
                            GlobalSetting.AUDIO_LANGUAGE[i]
                        )
                        audios_default_value_track_name.append(
                            GlobalSetting.AUDIO_TRACK_NAME[i]
                        )
                        audios_default_value_set_default.append(
                            GlobalSetting.AUDIO_SET_DEFAULT[i]
                        )
                ...
                warning_dialog = WarningDialog(
                    window_title="Audio Info", info_message="No audio found!", parent=self
                )
                warning_dialog.execute()
            ...
            if self.data[row_index].error_occurred:
                ...
                error_muxing_dialog = ErrorMuxingDialog(
                    window_title="Muxing Error", info_message=message, parent=self
                )
        ...
        if (
            ...
            output_video_name = Path(
                change_file_extension_to_mkv_with_random_suffix(
                    self.data[job_index].video_name
                )
            )
            output_video_name_absolute = os.path.join(folder_path, output_video_name)
            original_output_video_name = Path(
                get_file_name_with_mkv_extension(self.data[job_index].video_name)
            )
            output_file_name_folder_path_absolute = (
                self.get_output_file_name_folder_path_absolute(job_index)
            )
            original_file_name_absolute_path = os.path.join(
                output_file_name_folder_path_absolute, original_output_video_name
            )
    ...
    def rename_output_file_if_needed(self, job_index):
        if self.data[job_index].is_crc_calculating_required:
            ...
            file_name_with_crc = Path(
                get_file_name_with_crc(output_video_name, crc_string)
            )
            output_file_name_folder_path_absolute = (
                self.get_output_file_name_folder_path_absolute(job_index)
            )
            file_name_with_crc_absolute_path = os.path.join(
                output_file_name_folder_path_absolute, file_name_with_crc
            )
            ...
            file_name_old_absolute_path = os.path.join(
                output_file_name_folder_path_absolute, output_video_name
            )
            ...
            file_name_absolute_path = os.path.join(
                output_file_name_folder_path_absolute, output_video_name
            )
    ...
    def set_job_status_bad(self, row_index):
        ...
        self.setCellWidget(
            row_index,
            self.column_ids["Status"],
            ErrorCell(tool_tip="Error Happened\nDouble click for " "more details"),
        )
