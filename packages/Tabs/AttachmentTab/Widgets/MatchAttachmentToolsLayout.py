...
from packages.Tabs.AttachmentTab.Widgets.DeleteAttachmentButton import (
    DeleteAttachmentButton,
)
from packages.Tabs.AttachmentTab.Widgets.MoveAttachmentBottomButton import (
    MoveAttachmentBottomButton,
)
from packages.Tabs.AttachmentTab.Widgets.MoveAttachmentDownButton import (
    MoveAttachmentDownButton,
)
from packages.Tabs.AttachmentTab.Widgets.MoveAttachmentToButton import (
    MoveAttachmentToButton,
)
from packages.Tabs.AttachmentTab.Widgets.MoveAttachmentTopButton import (
    MoveAttachmentTopButton,
)
from packages.Tabs.AttachmentTab.Widgets.MoveAttachmentUpButton import (
    MoveAttachmentUpButton,
)
...
def update_global_attachment_files_list_order_to_top(index_to_move):
    while index_to_move > 0:
        ...
        GlobalSetting.ATTACHMENT_PATH_DATA_LIST[index_to_move] = (
            GlobalSetting.ATTACHMENT_PATH_DATA_LIST[index_to_move - 1]
        )
...
def update_global_attachment_files_list_order_to_position(list_of_old_and_new_index):
    ...
    GlobalSetting.ATTACHMENT_PATH_DATA_LIST[old_index] = (
        GlobalSetting.ATTACHMENT_PATH_DATA_LIST[new_index]
    )
...
class MatchAttachmentToolsLayout(QVBoxLayout):
    ...
    def connect_signals(self):
        self.move_attachment_up_button.swap_happened_signal.connect(
            self.refresh_attachment_table
        )
        self.move_attachment_up_button.selected_row_after_swap.connect(
            self.change_selected_attachment_row
        )
        self.move_attachment_top_button.swap_happened_signal.connect(
            self.refresh_attachment_table
        )
        self.move_attachment_top_button.selected_row_after_swap.connect(
            self.change_selected_attachment_row
        )
        self.move_attachment_down_button.swap_happened_signal.connect(
            self.refresh_attachment_table
        )
        self.move_attachment_down_button.selected_row_after_swap.connect(
            self.change_selected_attachment_row
        )
        self.move_attachment_bottom_button.swap_happened_signal.connect(
            self.refresh_attachment_table
        )
        self.move_attachment_bottom_button.selected_row_after_swap.connect(
            self.change_selected_attachment_row
        )
        self.move_attachment_to_button.swap_happened_signal.connect(
            self.refresh_attachment_table
        )
        self.move_attachment_to_button.selected_row_after_swap.connect(
            self.change_selected_attachment_row
        )
        ...
        self.delete_attachment_button.selected_row_after_delete.connect(
            self.change_selected_attachment_row
        )
    ...
    def setup_shortcuts(self):
        self.move_attachment_up_shortcut = QShortcut(
            QKeySequence("Ctrl+Up"), self.attachment_tab
        )
        self.move_attachment_up_shortcut.activated.connect(
            self.move_attachment_up_button.clicked_button
        )

        self.move_attachment_down_shortcut = QShortcut(
            QKeySequence("Ctrl+Down"), self.attachment_tab
        )
        self.move_attachment_down_shortcut.activated.connect(
            self.move_attachment_down_button.clicked_button
        )

        self.move_attachment_top_shortcut = QShortcut(
            QKeySequence("Ctrl+PgUp"), self.attachment_tab
        )
        self.move_attachment_top_shortcut.activated.connect(
            self.move_attachment_top_button.clicked_button
        )

        self.move_attachment_bottom_shortcut = QShortcut(
            QKeySequence("Ctrl+PgDown"), self.attachment_tab
        )
        self.move_attachment_bottom_shortcut.activated.connect(
            self.move_attachment_bottom_button.clicked_button
        )

        self.move_attachment_to_shortcut = QShortcut(
            QKeySequence("Ctrl+M"), self.attachment_tab
        )
        self.move_attachment_to_shortcut.activated.connect(
            self.move_attachment_to_button.clicked_button
        )

        self.delete_attachment_shortcut = QShortcut(
            QKeySequence("Delete"), self.attachment_tab
        )
        self.delete_attachment_shortcut.activated.connect(
            self.delete_attachment_button.clicked_button
        )
    ...
    def change_selected_attachment_row(self, new_selected_index):
        new_selected_index = min(
            new_selected_index, len(GlobalSetting.ATTACHMENT_PATH_DATA_LIST) - 1
        )
