...
from packages.Tabs.ChapterTab.Widgets.MoveChapterBottomButton import (
    MoveChapterBottomButton,
)
...
def update_global_chapter_files_list_order_to_top(index_to_move):
    while index_to_move > 0:
        ...
        GlobalSetting.CHAPTER_FILES_LIST[index_to_move] = (
            GlobalSetting.CHAPTER_FILES_LIST[index_to_move - 1]
        )
        ...
        GlobalSetting.CHAPTER_FILES_ABSOLUTE_PATH_LIST[index_to_move] = (
            GlobalSetting.CHAPTER_FILES_ABSOLUTE_PATH_LIST[index_to_move - 1]
        )
...
def update_global_chapter_files_list_order_to_up(index_to_move):
    temp_for_swap = GlobalSetting.CHAPTER_FILES_LIST[index_to_move]
    GlobalSetting.CHAPTER_FILES_LIST[index_to_move] = GlobalSetting.CHAPTER_FILES_LIST[
        index_to_move - 1
    ]
    GlobalSetting.CHAPTER_FILES_LIST[index_to_move - 1] = temp_for_swap
...
def update_global_chapter_files_list_order_to_position(list_of_old_and_new_index):
    ...
    GlobalSetting.CHAPTER_FILES_LIST[old_index] = GlobalSetting.CHAPTER_FILES_LIST[
        new_index
    ]
...
class MatchChapterToolsLayout(QVBoxLayout):
    ...
    def connect_signals(self):
        self.move_chapter_up_button.swap_happened_signal.connect(
            self.refresh_chapter_table
        )
        self.move_chapter_up_button.selected_row_after_swap.connect(
            self.change_selected_chapter_row
        )
        self.move_chapter_top_button.swap_happened_signal.connect(
            self.refresh_chapter_table
        )
        self.move_chapter_top_button.selected_row_after_swap.connect(
            self.change_selected_chapter_row
        )
        self.move_chapter_down_button.swap_happened_signal.connect(
            self.refresh_chapter_table
        )
        self.move_chapter_down_button.selected_row_after_swap.connect(
            self.change_selected_chapter_row
        )
        self.move_chapter_bottom_button.swap_happened_signal.connect(
            self.refresh_chapter_table
        )
        self.move_chapter_bottom_button.selected_row_after_swap.connect(
            self.change_selected_chapter_row
        )
        self.move_chapter_to_button.swap_happened_signal.connect(
            self.refresh_chapter_table
        )
        self.move_chapter_to_button.selected_row_after_swap.connect(
            self.change_selected_chapter_row
        )
        self.move_chapter_top_button.move_chapter_to_top_signal.connect(
            update_global_chapter_files_list_order_to_top
        )
        self.move_chapter_up_button.move_chapter_to_up_signal.connect(
            update_global_chapter_files_list_order_to_up
        )
        ...
        self.delete_chapter_button.selected_row_after_delete.connect(
            self.change_selected_chapter_row
        )
    ...
    def setup_shortcuts(self):
        self.move_chapter_up_shortcut = QShortcut(
            QKeySequence("Ctrl+Up"), self.chapter_tab
        )
        self.move_chapter_up_shortcut.activated.connect(
            self.move_chapter_up_button.clicked_button
        )

        self.move_chapter_down_shortcut = QShortcut(
            QKeySequence("Ctrl+Down"), self.chapter_tab
        )
        self.move_chapter_down_shortcut.activated.connect(
            self.move_chapter_down_button.clicked_button
        )

        self.move_chapter_top_shortcut = QShortcut(
            QKeySequence("Ctrl+PgUp"), self.chapter_tab
        )
        ...
        self.move_chapter_to_shortcut = QShortcut(
            QKeySequence("Ctrl+M"), self.chapter_tab
        )
        ...
        self.delete_chapter_shortcut.activated.connect(
            self.delete_chapter_button.clicked_button
        )
    ...
    def change_selected_chapter_row(self, new_selected_index):
        new_selected_index = min(
            new_selected_index, len(GlobalSetting.CHAPTER_FILES_LIST) - 1
        )
