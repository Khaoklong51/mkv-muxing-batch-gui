...
class TrackInfoTable(TableWidget):
    ...
    def setup_horizontal_header(self):
        self.horizontal_header.setSectionResizeMode(
            self.column_ids.Video_Name, QHeaderView.ResizeMode.Interactive
        )
        self.horizontal_header.setSectionResizeMode(
            self.column_ids.Found, QHeaderView.ResizeMode.Fixed
        )
        self.horizontal_header.setSectionResizeMode(
            self.column_ids.Is_Default, QHeaderView.ResizeMode.Fixed
        )
        self.horizontal_header.setSectionResizeMode(
            self.column_ids.Is_Forced, QHeaderView.ResizeMode.Fixed
        )
        self.horizontal_header.setSectionResizeMode(
    ...
    def set_row_value_found_track(self, row_id, found):
        if found:
            self.setCellWidget(
                row_id, self.column_ids.Found, GreenTikCell(tool_tip="Track Found")
            )
        else:
            self.setCellWidget(
                row_id, self.column_ids.Found, RedCrossCell(tool_tip="Track Not Found")
            )
    ...
    def set_row_value_is_default_track(self, row_id, is_default):
        if is_default:
            self.setCellWidget(
                row_id,
                self.column_ids.Is_Default,
                GreenTikCell(tool_tip="The track is default"),
            )
        else:
            self.setCellWidget(
                row_id,
                self.column_ids.Is_Default,
                RedCrossCell(tool_tip="The track is not default"),
            )
    ...
    def set_row_value_is_forced_track(self, row_id, is_forced):
        if is_forced:
            self.setCellWidget(
                row_id,
                self.column_ids.Is_Forced,
                GreenTikCell(tool_tip="The track is forced"),
            )
        else:
            self.setCellWidget(
                row_id,
    ...
    def check_if_video_name_need_resize_column_to_fit_content(self):
        ...
        for i in range(self.rowCount()):
            ...
            new_column_width = max(
                new_column_width,
                column_font_metrics.horizontalAdvance(str(self.video_names[i])),
            )
        new_column_width += 10
