...
class MatchAudioToolsLayout(QVBoxLayout):
    ...
    def connect_signals(self):
        ...
        self.move_audio_up_button.selected_row_after_swap.connect(
            self.change_selected_audio_row
        )
        self.move_audio_top_button.swap_happened_signal.connect(self.refresh_audio_table)
        self.move_audio_top_button.selected_row_after_swap.connect(
            self.change_selected_audio_row
        )
        self.move_audio_top_button.move_audio_to_top_signal.connect(
            self.update_global_audio_files_list_order_to_top
        )
        self.move_audio_up_button.move_audio_to_up_signal.connect(
            self.update_global_audio_files_list_order_to_up
        )
        ...
        self.move_audio_down_button.selected_row_after_swap.connect(
            self.change_selected_audio_row
        )
        self.move_audio_bottom_button.swap_happened_signal.connect(
            self.refresh_audio_table
        )
        self.move_audio_bottom_button.selected_row_after_swap.connect(
            self.change_selected_audio_row
        )
        self.move_audio_to_button.swap_happened_signal.connect(self.refresh_audio_table)
        self.move_audio_to_button.selected_row_after_swap.connect(
            self.change_selected_audio_row
        )
        self.delete_audio_button.delete_happened_signal.connect(
            self.update_global_audio_files_list_order_deleting
        )
        self.delete_audio_button.selected_row_after_delete.connect(
            self.change_selected_audio_row
        )
    ...
    def setup_shortcuts(self):
        ...
        self.move_audio_up_shortcut.activated.connect(
            self.move_audio_up_button.clicked_button
        )

        self.move_audio_down_shortcut = QShortcut(
            QKeySequence("Ctrl+Down"), self.audio_tab
        )
        self.move_audio_down_shortcut.activated.connect(
            self.move_audio_down_button.clicked_button
        )

        self.move_audio_top_shortcut = QShortcut(
            QKeySequence("Ctrl+PgUp"), self.audio_tab
        )
        self.move_audio_top_shortcut.activated.connect(
            self.move_audio_top_button.clicked_button
        )

        self.move_audio_bottom_shortcut = QShortcut(
            QKeySequence("Ctrl+PgDown"), self.audio_tab
        )
        self.move_audio_bottom_shortcut.activated.connect(
            self.move_audio_bottom_button.clicked_button
        )

        self.move_audio_to_shortcut = QShortcut(QKeySequence("Ctrl+M"), self.audio_tab)
        self.move_audio_to_shortcut.activated.connect(
            self.move_audio_to_button.clicked_button
        )

        self.delete_audio_shortcut = QShortcut(QKeySequence("Delete"), self.audio_tab)
        self.delete_audio_shortcut.activated.connect(
            self.delete_audio_button.clicked_button
        )
    ...
    def change_selected_audio_row(self, new_selected_index):
        new_selected_index = min(
            new_selected_index, len(GlobalSetting.AUDIO_FILES_LIST[self.tab_index]) - 1
        )
    ...
    def update_global_audio_files_list_order_to_top(self, index_to_move):
        while index_to_move > 0:
            ...
            temp_for_swap = GlobalSetting.AUDIO_FILES_ABSOLUTE_PATH_LIST[self.tab_index][
                index_to_move
            ]
